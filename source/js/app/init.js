(function(w, $){
	...
	$(document).ready(function(){
        // Initialize responsive video components
        $('[data-simple-retirement-range]').each(function () {
            var newSimpleRetirementRange = new nflpbo.Comp.SimpleRetirementRangeInput({
                $el: $(this)
            });
        });
        
        // Initialize responsive video components
        $('[data-responsive-video]').each(function () {
            var newResponsiveVideo = new nflpbo.Comp.ResponsiveVideo({
                $el: $(this),
                videolURL: $(this).data('videol'),
                videomURL: $(this).data('videom'),
                videosURL: $(this).data('videos')
            });
        });
        
        $('[data-activetoggle]').each(function() {
            var newActiveToggle = new nflpbo.Comp.ActiveToggle({
                $el: $(this),
                $activeToggleSection: $(this).find('[data-activetogglesection]'),
                $activeTrigger: $(this).find('[data-activetrigger]'),
                $independentTrigger: $(this).find('[data-independenttrigger]'),
                $independentSections: $(this).find('[data-independentsection]').addBack('[data-independentsection]'),
                allowNone: $(this).data('allownone')
            });
        });

        //replace accordion function with css transition
		...
		// Fixing foundation equalizer + interchange bugc
