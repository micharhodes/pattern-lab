# SWX Fork of Pattern Lab Node
This project is forked from the Node version of [Pattern Lab](http://patternlab.io/). The original readme is preserved below starting with the "About" section and is relevant for the majority of the development process. 

The project is already customized to use the gulp workflow. Several Node dependencies have been added to facilitate SWX / Armstrong project needs and have been added to the `package.json` file. 

Getting Started:

1. Install [NodeJS](https://nodejs.org/en/download/) and include Node Package Manager (NPM) during install.
2. Install [Visual C++ Redistributable for Visual Studio 2015](https://www.microsoft.com/en-us/download/details.aspx?id=48145)
3. From your working directory run `npm install`

CI Build:

* Run `gulp` from the command line.
* Build output is the `.\public\` folder which should be the webroot. Any HTTP server can be used as the content is static.

DEV Build (includes simple webserver and browsersync):

* Run `gulp serve` from the command line.  
* Changes will be picked up by gulp.watch and browsers will be automaticly refreshed.

Notes:

* If the `gulp` command is unrecognized, then you may need to install the GULP npm package globally.  This can be done by running `npm install gulp -g` from the command line. 
